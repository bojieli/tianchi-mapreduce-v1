...
public class MyReducer implements Reducer {
    ...
    public void reduce(Record key, Iterator<Record> values, TaskContext context) throws IOException {
    	...
    	while(values.hasNext())
    	{
    		...
    		//count action num
    		bro_decay_cnt += val.getBigint("ui_bro") * decay(idays, labelday, bro_type) * double12(idays, bro_type);
    		fav_decay_cnt += val.getBigint("ui_fav") * decay(idays, labelday, fav_type) * double12(idays, fav_type);
    		cart_decay_cnt += val.getBigint("ui_cart") * decay(idays, labelday, cart_type) * double12(idays, cart_type);
    		buy_decay_cnt += val.getBigint("ui_buy") * decay(idays, labelday, buy_type) * double12(idays, buy_type);
