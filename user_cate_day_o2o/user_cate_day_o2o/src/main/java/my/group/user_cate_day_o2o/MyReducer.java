package my.group.user_cate_day_o2o;
...
public class MyReducer implements Reducer {
    ...
    public void reduce(Record key, Iterator<Record> values, TaskContext context) throws IOException {
    	...
    	boolean isbuycate24h = false, isbuycate3d = false, isbuycate5d = false;
    	while(values.hasNext())
    	{
    		Record val = values.next();
    		int idays = val.getBigint("curday").intValue();
    		
    		bro = val.getBigint("uc_bro");
    		fav = val.getBigint("uc_fav");
    		cart = val.getBigint("uc_cart");
    		buy = val.getBigint("uc_buy");
    		...
    		if(buy == 1L)
    		{
    			...
    			if(!isbuycate24h && daygap <= 1)
    				isbuycate24h = true;
    			if(!isbuycate3d && daygap <=3)
    				isbuycate3d = true;
    			if(!isbuycate5d && daygap <=5)
    				isbuycate5d = true;
    		}
    	...
    	result.setDouble("uc_bro_decay_cnt", bro_decay_cnt);
    	result.setDouble("uc_fav_decay_cnt", fav_decay_cnt);
    	result.setDouble("uc_cart_decay_cnt", cart_decay_cnt);
